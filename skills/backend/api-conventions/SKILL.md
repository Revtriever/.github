...
- HTTP input: zod schema per endpoint (`create-charge.schema.ts` in the module's `validation/`),
  wired via nestjs-zod pipe. The inferred type IS the DTO — no class-validator, no duplicated
  interfaces.
- Queue payloads and gateway webhooks: same pattern — parse at the boundary, trust types inside.
...
- Errors: `@ApiProblemResponse({ status, description, example? })` for every RevError the endpoint
...
**Errors carry keys, never user text.** A `RevError` has `code`, `status`, `meta` and `cause` — no
...
- Domain errors are typed classes (`ChargeAlreadyRecoveredError`) thrown by services/applications;
  a single global exception filter maps them to HTTP. Controllers never try/catch business errors.
- Never leak internals: stack traces, SQL, gateway raw errors go to logs — the response gets the
  problem+json. `meta` is serialized into the response, so a provider's raw error body must never
  be put there.
- Expected business outcomes ("charge already paid") are NOT exceptions when they're a normal
