...
name: data-access
description: >
  Database access rules for revtriever-api: Kysely, repositories, BaseRepository with automatic
...
- Table names: **PascalCase plural** — `Charges`, `Companies`, `GatewayConnections`.
- Columns: **camelCase** — `companyId`, `nextActionAt`. (Kysely quotes identifiers automatically;
...
- Gateway payloads/raw events: `jsonb`.
...
- Hard delete exists only for compliance flows (LGPD erasure) — named `hardDelete`, and every call
  site needs a comment explaining why.
...
export class ChargesRepository extends TenantRepository<'Charges'> { ... }
...
- Repository methods speak domain language (`findDueForRetry`, `markRecovered`), not SQL language
...
- Transactions wrap **state transitions**, not whole flows: keep them short, never hold one across
  an external HTTP call (gateway APIs) or queue publish.
- Concurrency on the dunning sweep: `FOR UPDATE SKIP LOCKED` — workers pick disjoint rows, no
...
- Files in `migrations/`, TypeScript, `up`/`down`, named `<timestamp>_<verb>-<subject>.ts`
  (`..._create-charges.ts`, `..._add-next-action-at-to-charges.ts`).
