...
| `negotiation-policy`          | Negotiation policy published for a connection                                    | `connectionId`, `kind`, `version`
| `negotiation-interview`       | Policy interview: every turn (`stage` opened/turn/completed), budget refusal, round cap, tool failure | `connectionId`, `stage`, `turn`, `used`, `pending`, `degraded`, `done`, `durationMs`, `rounds`, `inputTokens`, `outputTokens`, `cacheReadTokens`, `cacheWriteTokens`, `tool`
| `negotiation-simulation`      | Simulated payer turn, budget refusal                                             | `connectionId`, `scenario`, `outcome`, `used`, `degraded`, `durationMs`, `rounds`, `inputTokens`, `outputTokens`
