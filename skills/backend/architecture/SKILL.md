...
name: architecture
description: >
  Layered architecture and module rules for revtriever-api. Use this skill whenever creating a new
...
| Domain      | Its OWN repository, CHILD domains (e.g. CampaignDomain → CampaignRecipientDomain) | Repositories of other aggregates, applications, foreign modules |
...
package, not in `src/`. Today: `@revtriever/gateways` (payment gateway adapters).
...
- **Neutral errors**: it throws its own error type (`GatewayError`), never a `RevError`. The app
...
touches the outside world: cache, payment gateways, queue producer, message senders, clock.
...
```typescript
// config/asaas.config.ts
const schema = z.object({
  ASAAS_BASE_URL: z.string().url(),
  ASAAS_TIMEOUT_MS: z.coerce.number().default(10_000)
});

export const asaasConfig = registerAs('asaas', () => schema.parse(process.env));
export type AsaasConfig = z.infer<typeof schema>;
```

- Boot fails fast on invalid/missing env — never a runtime surprise.
- A module injects `asaasConfig.KEY` via `ConfigType<typeof asaasConfig>` — never `ConfigService.get('...')`
...
The first fully-wired module is the reference for new ones. Until it exists, this file is the
authority. When it lands, add the pointer here: `src/modules/<feature>/`.
