...
name: caching
description: >
  Caching rules for revtriever-api: CachePort, in-memory driver, key naming, TTLs, and what must
...
- Gateway API access tokens (their TTL is the cache TTL, minus a safety margin).
- Read-mostly config resolved per company (recovery policy, feature toggles) — short TTL (60s),
...
`<area>:<companyId>:<rest>` — e.g. `policy:0198f3aa-...:active`, `asaas-token:0198f3aa-...`.
