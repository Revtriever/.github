...
name: infrastructure
description: >
  AWS infrastructure (CDK) and CI/CD (GitHub Actions) rules for revtriever-api: accounts,
...
# Infrastructure & CI/CD — revtriever-api
...
2. **Real secrets → SSM Parameter Store** (SecureString, path `/revtriever/<name>`), referenced by
...
3. **Customer gateway credentials are DATA, not config**: Postgres + KMS envelope encryption.
   Never in SSM, never in env, never in logs.
...
- Budget alarm exists (US$ 40/mo, management account). Treat approaching it as a design smell.
