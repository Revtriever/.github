...
name: testing
description: >
  Test strategy for revtriever-api: unit vs integration split, Testcontainers Postgres with
...
- Mock at **port boundaries only** (repositories, CachePort, MailPort, QueuePublisherPort,
  gateway ports). Mocking a module's private service from another test is forbidden — test through
...
2. **Template database**: migrations run ONCE into `revtriever_template`; each Vitest worker gets
   `CREATE DATABASE test_w<N> TEMPLATE revtriever_template` (~tens of ms) instead of re-migrating.
