...
could not run through no fault of the code — a Meta template still under review, a gateway key
missing from `.env`, a sandbox that is down. **Blocked is never a pass and never a bug.** Collapsing
...
words — *teto*, *transbordo*, *concessão*, *régua* — and the PRD is where they mean something.
...
repos: bring both branches up (`revtriever-api` and `revtriever-fe`), and point the FE at the local
API.
...
**Safety, non-negotiable.** Local only, gateway sandboxes only, and the dev send-guard stays on.
Never point a round at production, and never touch a production connection — there is exactly one
(Zoppy/Vindi) and its régua is paused on purpose. A QA round that duns a real payer is an incident.
...
| a gateway event arrives | `npm run webhook:replay -- <fixture>` |
...
`requestId`, the Mailpit message, the gateway's response body. A pass with no evidence is an
opinion.
